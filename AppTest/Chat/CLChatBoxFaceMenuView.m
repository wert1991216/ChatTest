...
@interface CLChatBoxFaceMenuView ()
...
@implementation CLChatBoxFaceMenuView
...
//    [self.scrollView setFrame:CGRectMake(w + 0.5, 0, self.frameWidth - self.addButton.frameWidth, self.frameHeight)];
    [self.scrollView setFrame:CGRectMake(0, 0, self.frameWidth - self.sendButton.frameWidth, self.frameHeight)];
    [self.scrollView setContentSize:CGSizeMake(w * (faceGroupArray.count + 3), self.scrollView.frameHeight)];
    float x = 0;
    int i = 0;
    for (CLFaceGroup *group in faceGroupArray) {
...
//        if ([[_faceGroupArray objectAtIndex:sender.tag] faceType] == CLFaceTypeEmoji) {
//            [self addSubview:self.sendButton];
